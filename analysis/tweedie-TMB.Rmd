...
## Data and parameters
data <- list(y = y, x = x)
parameters <- list(a=0, b=0, p = 0, log_phi = log(1) )
...
nll_tweedie <- function(par) {
  ...
  mu <- exp(eta)
  phi <- exp(par[4])
  power <- plogis(par[3]) + 1
  # sum of negative log likelihoods:
  loglik <- log(dtweedie(y, power = power, mu = mu, phi = phi))
  nll <- -sum(loglik) #[is.finite(loglik)]
  print(nll)
  nll
}

tweedie_ml <- nlminb(start = c(a = 0, b = 0,  logis_p = 0, log_phi = log(1)), nll_tweedie)
tweedie_ml$par 
```
