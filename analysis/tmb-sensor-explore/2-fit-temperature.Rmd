...
>>>>>>> 94325345634652c6249d3c01d19d68b896540f18
cv_args1 <- cv_args
cv_loglik <- c()
...
# readRDS(file = "sdmTMB_cv_nontv.rds")
=======
>>>>>>> 94325345634652c6249d3c01d19d68b896540f18
```
...
```{r}
predictions_nd <- predict(m_temp2_rw_nosp, newdata = qcs_grid)
...
Misc models
```{r}
temp2 <- sdmTMB_cv(data = d_trawl, n_knots = 30, x = "X", y ="Y", k_folds = 5, plot_spde = TRUE,
                   formula = temperature_c ~ 0 + as.factor(year) + depth_scaled + depth_scaled2,
                   time = "year", family = gaussian(link = "identity"), 
                   ar1_fields = FALSE, include_spatial = FALSE)
plot_cv_meshes(temp2)
sum(temp2$sum_loglik)

temp3 <- sdmTMB_cv(data = d_trawl, n_knots = 30, x = "X", y ="Y", k_folds = 5, plot_spde = TRUE,
                   formula = temperature_c ~ 0 + as.factor(year) + depth_scaled + depth_scaled2 + depth_scaled3,
                   time = "year", family = gaussian(link = "identity"), 
                   ar1_fields = FALSE, include_spatial = FALSE)
sum(temp3$sum_loglik)

# allow slope with depth to vary through between years with a random walk
temp1_rw <- sdmTMB_cv(data = d_trawl, n_knots = 30, x = "X", y ="Y", k_folds = 5, plot_spde = TRUE,
                   formula = temperature_c ~ 0 + as.factor(year),
                   time_varying = ~ 0 + depth_scaled,
                   time = "year", family = gaussian(link = "identity"), 
                   ar1_fields = FALSE, include_spatial = FALSE)
sum(temp1_rw$sum_loglik)

temp2_rw <- sdmTMB_cv(data = d_trawl, n_knots = 30, x = "X", y ="Y", k_folds = 5, plot_spde = TRUE,
                   formula = temperature_c ~ 0 + as.factor(year),
                   time_varying = ~ 0 + depth_scaled + depth_scaled2,
                   time = "year", family = gaussian(link = "identity"), 
                   ar1_fields = FALSE, include_spatial = FALSE)
sum(temp2_rw$sum_loglik)


temp3_rw <- sdmTMB_cv(data = d_trawl, n_knots = 30, x = "X", y ="Y", k_folds = 5, plot_spde = TRUE,
                   formula = temperature_c ~ 0 + as.factor(year),
...
# there is probably a problem with doing this... but need to figure out how to add poly args to predict function
```{r eval=FALSE}
