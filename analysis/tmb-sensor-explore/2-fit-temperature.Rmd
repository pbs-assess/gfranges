...
```{r}
spde2 <- make_spde(d_trawl2$X, d_trawl2$Y, n_knots = 500)
plot_spde(spde2)
```

```{r eval=FALSE}
m_temp2 <- sdmTMB(
  temperature_c ~ 0 + as.factor(year) + DOY_scaled * shallow + DOY_scaled2 * shallow,
  data = d_trawl2,
  time_varying = ~ 0 + depth_scaled + depth_scaled2,
  ar1_fields = TRUE, include_spatial = TRUE,
  time = "year", spde = spde2, family = gaussian(link = "identity"),
  nlminb_loops = 2,
  newton_steps = 1,
  silent = TRUE
)

saveRDS(m_temp2, file = (here::here("analysis/tmb-sensor-explore/models/model-temp-all-years-DOY.rds")))
...
```{r eval=FALSE}
m_temp3 <- sdmTMB(
  temperature_c ~ 0 + as.factor(year) + SST_scaled * shallow + DOY_scaled * shallow + DOY_scaled2 * shallow,
  data = d_trawl2,
  time_varying = ~ 0 + depth_scaled + depth_scaled2,
  ar1_fields = TRUE, include_spatial = TRUE,
  time = "year", spde = spde2, family = gaussian(link = "identity"),
...
```{r eval=FALSE}
get_diag(m_temp3, response = "temperature_c", variable = "depth_scaled")
```
...
unique(nd_all$year)
glimpse(nd_all)
pred_temp <- predict(m_temp_rw, newdata = nd_all)

# saveRDS(pred_temp, file = "../VOCC/data/predicted_temp_allyears_revised2")
```

```{r}
p_nd <- readRDS("../VOCC/data/predicted_temp_allyears_revised2")
