...
Load data
```{r}
# d_trawl1 <- readRDS(here::here("analysis/tmb-sensor-explore/data/sensor-data-processed.rds"))
# nrow(d_trawl1)
d_trawl <- readRDS(here::here("analysis/tmb-sensor-explore/data/dat-sensor-trawl-SST.rds"))
nrow(d_trawl)
# d_trawl$log_temperature_c <- log(d_trawl$temperature_c)
unique(d_trawl$year)
d_trawl[is.na(d_trawl$year), ]$year <- 2003
d_trawl <- d_trawl[!is.na(d_trawl$depth_m), ]
...
d_trawl <- mutate(d_trawl,
  ...
  SST_scaled = arm::rescale(SST),
  depth_scaled2 = depth_scaled^2,
  depth_scaled3 = depth_scaled^3,
  shallow = ifelse(depth_m>50,0,1)
)
glimpse(d_trawl)
```
...
```{r}
ggplot(dplyr::filter(d_trawl, depth_m <50), aes(SST,temperature_c, colour = depth_m)) + geom_point()
```
...
m_temp3 <- sdmTMB(d_trawl,
  temperature_c ~ 0 + as.factor(year) + SST_scaled*shallow + depth_scaled + depth_scaled2 + depth_scaled3,
  ar1_fields = FALSE, include_spatial = TRUE,
  time = "year", spde = spde, family = gaussian(link = "identity"),
  silent = TRUE
)

m_temp3_nosst <- sdmTMB(d_trawl,
...
predictions3b$data$residuals <- residuals(m_temp3_nosst)

nd <- data.frame(sst_resid = predictions3$data$residuals, nosst_resid = predictions3b$data$residuals, shallow = d_trawl$shallow, depth = d_trawl$depth_m)
...
cv_args <- generate_cv_args(
  formula = list(
    temperature_c ~ 0 + as.factor(year) + SST + depth_scaled + depth_scaled2 + depth_scaled3
  ),
...
cv_args <- generate_cv_args(
  formula = list(
    temperature_c ~ 1 + SST + depth_scaled,
    temperature_c ~ 1 + SST + depth_scaled + depth_scaled2,
    temperature_c ~ 1 + SST + depth_scaled + depth_scaled2 + depth_scaled3,
    temperature_c ~ 0 + as.factor(year) + depth_scaled,
    temperature_c ~ 0 + as.factor(year) + depth_scaled + depth_scaled2,
    temperature_c ~ 0 + as.factor(year) + depth_scaled + depth_scaled2 + depth_scaled3,
    temperature_c ~ 0 + as.factor(year) + SST + depth_scaled,
    temperature_c ~ 0 + as.factor(year) + SST + depth_scaled + depth_scaled2,
    temperature_c ~ 0 + as.factor(year) + SST + depth_scaled + depth_scaled2 + depth_scaled3,
    temperature_c ~ 0 + as.factor(year) + SST * depth_scaled,
    temperature_c ~ 0 + as.factor(year) + SST * depth_scaled + SST * depth_scaled2,
    temperature_c ~ 0 + as.factor(year) + SST * depth_scaled + SST * depth_scaled2 + SST * depth_scaled3
  ),
  time_varying = list(NULL),
  ar1_fields = c(FALSE),
  include_spatial = c(FALSE),
...
cv_args <- generate_cv_args(
  formula = list(
    temperature_c ~ 0 + as.factor(year) + SST,
...
write.csv(more_knots$summary, file = "cv-output/cv-more-seeds.csv")
saveRDS(more_knots, file = "cv-output/cv-more-seeds.rds")
glimpse(more_knots$summary)
```

## Change seeds 
Also increased knot number too 
Model without SST is always very slightly higher ll
...
temp_rw <- sdmTMB_cv(
  ...
  formula = temperature_c ~ 0 + as.factor(year) + SST,
...
Does a fixed interaction with SST do any better?
