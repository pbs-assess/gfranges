...
# nrow(d_trawl1)
d_trawl <- readRDS(here::here("analysis/tmb-sensor-explore/data/dat-sensor-trawl-meanSST.rds"))
nrow(d_trawl)
...
```{r}
ggplot(dplyr::filter(d_trawl, depth_m <60), aes(SST,temperature_c, colour = -depth_m)) + geom_point()
```
...
```{r}
ggplot(dplyr::filter(d_trawl, depth_m <50), aes(meanSST,temperature_c, colour = -depth_m)) + geom_point()
```
