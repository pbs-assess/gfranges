...
```{r}
spde <- make_spde(d_trawl$X, d_trawl$Y, n_knots = 400)
plot_spde(spde)
```
...
m_temp_rw <- sdmTMB(d_trawl,
  ...
  ar1_fields = TRUE, include_spatial = TRUE, # changed AR1_field to true
  time = "year", spde = spde, family = gaussian(link = "identity"),
  silent = FALSE
)

saveRDS(m_temp_rw, file = (here::here("analysis/tmb-sensor-explore/models/model-temp-all-years-400kn.rds")))
...
saveRDS(predict_temp, file = "../VOCC/data/predicted_temp_allyears_revised")
p_nd <- predict_temp
