...
d_trawl1 <- d_trawl %>% filter(!(year == 2016 & ssid ==4)) #%>% filter(ssid != 16) %>% filter(ssid != 4)    
#test <- d_trawl1 %>% filter(year == 2016 & ssid ==16) 
spde <- make_spde(d_trawl1$X, d_trawl1$Y, n_knots = 800) ### TODO: rerun with 500
plot_spde(spde)
```
...
```{r eval=FALSE}
temperature <- readRDS(here::here("analysis/VOCC/data/predicted_temp_allyears_revised2"))
...
Other models tried:
```{r eval=FALSE}
do_model_DOY <- readRDS(here::here("analysis/tmb-sensor-explore/models/model-do-without-wcvi2016.rds"))
do_model_DOY
...
```{r eval=FALSE}
do_model_all <- readRDS(here::here("analysis/tmb-sensor-explore/models/model-do-without-wcvi2016-poly.rds"))
p_nd <- predict(do_model_all_DOY, newdata = newdata)
...
Save new predictions without QCS gap and WCVI 2016 DO
```{r eval=FALSE}
...
# p_nd <- readRDS((here::here("analysis/VOCC/data/predicted-DO-new.rds")))

p_nd <- readRDS((here::here("analysis/VOCC/data/predicted-DO-june-2020.rds")))
