...
survs <- c('SYN QCS', 'SYN HS', 'SYN WCHG', 'SYN WCVI')[[1]]
...
d <- gfplot::get_survey_sets("pacific cod")

glimpse(d)

dat <- gfplot:::tidy_survey_sets(d, survs, years = seq(1, 1e6), # say which surveys and years to include
                                 density_column = col)
glimpse(dat)


#.y <- filter(d) %>% pull(year) %>% unique()
.y <- filter(d, survs  == 'SYN QCS') %>% pull(year) %>% unique()
#.y <- filter(d, survey_abbrev  == 'SYN WCVI') %>% pull(year) %>% unique()
...
for (i in seq_along(.y)) {
  fit[[i]] <- gfsynopsis::fit_survey_maps(dat,
  survs = c("SYN QCS", "SYN HS", "SYN WCHG", "SYN WCVI")[[1]],
  species = "pacific cod",
...
surveys <- c("SYN QCS", "SYN HS", "SYN WCHG", "SYN WCVI")[[1]]
