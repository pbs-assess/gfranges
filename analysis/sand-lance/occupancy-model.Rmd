...
mod.formula <- as.formula(paste0(
  "present ~ 0 + as.factor(year_pair) + 
  te(sandy_scaled, depth_scaled, k = 5) + 
  s(muddy_scaled, k = 3) + 
  s(temp_scaled, k = 3)"
  ))
clim_cov <- "temp"
...
```{r}
g2 <- ggplot(pred, 
      aes(est, residuals, colour = as.factor(present))) +
...
if(clim_cov == "temp"){
nd_temp <- data.frame(
  depth_scaled = min(data$depth_scaled)-0.2, # use min depth for estimated error at relevant depths
  sandy_scaled = 0,
  muddy_scaled = 0,
...
p_mean <- predictions  %>% group_by(X, Y) %>%
  ...
  ggtitle(paste0("Estimated biomass density of ", species, 
    " \n(max = ", max_prob, ")"))
print(p_mean)
ggsave(paste0("figs/map-", spp, covs, "-mean-distribution.png"), width = 5, height = 5)
```
