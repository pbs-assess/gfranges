...
```{r}
g2 <- ggplot(pred, 
      aes((est), residuals, colour = density)) +
...
(g_depth <- ggplot(p_depth, aes(depth, exp(est), 
  ...
  coord_cartesian(ylim = c(0, max(exp(p_depth$est))*20), xlim = c(0, 150)) +
...
if(clim_cov == "roms"){
nd_roms <- data.frame(
  depth_scaled = min(data$depth_scaled)-0.2, 
  sandy_scaled = 0,
  muddy_scaled = 0,
...
(g_temp <- ggplot(p_temp, aes(temp, exp(est), 
  ymin = exp(est - 1.96 * est_se), 
  ymax = exp(est + 1.96 * est_se))) +
  # coord_cartesian(ylim = c(0, max(exp(p_temp$est))*20)) +
  geom_line() + geom_ribbon(alpha = 0.4))
...
```{r}
if(covs %in% c("-sand-depth-roms", "-sand-depth-temp")){
nd_sand <- data.frame(
  depth_scaled = min(data$depth_scaled)-0.2, 
...
(g_sand <- ggplot(p_sand, aes(sandy, exp(est),
  ymin = exp(est - 1.96 * est_se), 
  ymax = exp(est + 1.96 * est_se))) +
  # coord_cartesian(ylim = c(0,max(exp(p_sand$est))*20)) +
  geom_line() + geom_ribbon(alpha = 0.4))

# ggplot(p_sand, aes(sandy, exp(est),
#   ymin = exp(est - 1.96 * est_se), 
#   ymax = exp(est + 1.96 * est_se))) +
#   coord_cartesian(ylim = c(0, max(exp(p_sand$est))*2), xlim = c(0,1)) +
#   geom_line() + geom_ribbon(alpha = 0.4)
...
```{r}
if(covs %in% c("-mud-depth-roms", "-mud-depth-temp")){
nd_mud <- data.frame(
  depth_scaled = min(data$depth_scaled)-0.2, 
...
(g_mud <- ggplot(p_mud, aes(muddy, exp(est),
  ymin = exp(est - 1.96 * est_se), 
  ymax = exp(est + 1.96 * est_se))) +
  # coord_cartesian(ylim = c(0,max(exp(p_mud$est))*20)) +
  geom_line() + geom_ribbon(alpha = 0.4))

# ggplot(p_mud, aes(muddy, exp(est),
#   ymin = exp(est - 1.96 * est_se), 
#   ymax = exp(est + 1.96 * est_se))) +
#   coord_cartesian(ylim = c(0, max(exp(p_mud$est))*2), xlim = c(0,1)) +
#   geom_line() + geom_ribbon(alpha = 0.4)
