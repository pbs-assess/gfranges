...
```{r}
fit_inla_sensor <- function(dat, response = "temperature_C_avg",
                     n_knots = 50,
                     family = "binomial",
                     max_edge = c(20, 100),
                     kmeans = FALSE,
...
pg <- gfplot:::make_prediction_grid(filter(temp_dat, year %in% c(2016, 2017)), survey ="SYN QCS")$grid


cell_area <- gfplot:::make_prediction_grid(filter(temp_dat, year %in% c(2016, 2017)), survey = "SYN QCS")$cell_area

pred_temp <- predict_inla_sens(obj=obj, pred_grid = pg , samples = 100L,
                         include_depth = TRUE)
```
...
predicted <- predict_inla_sens(obj = obj, pred_grid = pg_one_year, samples = 100, include_depth = TRUE)
```
...
predict_inla_sens <- function(obj, pred_grid, samples = 100L,
                         include_depth = TRUE) {
...
out_med <- apply(predicted, c(1), median)
# out_bin <- stats::plogis(p_bin)
# out_bin <- apply(out_bin, c(1, 3), median)
dim(predicted)

pg_all <- bind_rows(replicate(1, pg_one_year, simplify = FALSE))
