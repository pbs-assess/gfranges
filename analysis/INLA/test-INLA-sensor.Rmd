...
```{r}
ggplot(sens_dat, aes(depth)) + geom_histogram() 
ggplot(sens_dat, aes(density*1000)) + geom_histogram() 
ggplot(sens_dat, aes(depth, (density*1000))) + geom_point() 
...
fit_inla_sensor <- function(d, response = "temperature_C_avg",
   ...
   # create data.frame including response ("y"), coordinates("xcoo", "ycoo"), covariates, and "intercept"
  dat <- data.frame(
  ...
  #time_k <- dat$raw_time - min(dat$raw_time) + 1
  k <- max(time_k)
  
  time_lab <- paste0("Y", seq(1, k))
  dat[time_lab] <- 0
  dat[, time_lab[1]] <- 1
  for (j in seq_along(time_lab)) {
    dat[time_k == j, time_lab[j]] <- 1
  }
  #names(dat)[names(dat)=="Y1"] <- "intercept"
  
  ## Select time knots and generate time mesh
  t_knots = seq(1, k, length = k)
  mesh1 = inla.mesh.1d(loc = t_knots, degree = 1, boundary = "free")
  ...
  message("INLA max_edge = ", "c(", max_edge[[1]], ", ", max_edge[[2]], "), n = ", mesh$n)
  ...
  # construct index for ar1 model ---- DON'T QUITE UNDERSTAND THIS
...
#  
  ...
  formula <- y ~ -1 + depth_scaled+depth_scaled2+Y1+Y2+Y3+Y4+Y5+Y6 +
   f(spatial.field, model = spde, group = spatial.field.group, control.group = list(model = "ar1"))
...
# pg$year_k <- pg$year - min(pg$year) + 1
# count_k <- unique(year_k)
