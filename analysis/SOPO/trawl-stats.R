...
hbll1 <- filter(hbll, species_code == 394 & year == 2019)  

# hbll1 <- filter(hbll, species_common_name == "yelloweye rockfish" & year == 2019)  

hbll_tidy <- #gfplot::
  tidy_survey_sets(hbll1, survey = "HBLL INS N",
  years = 2019, density_column = "density_ppkm2") %>% mutate(akima_depth = depth)
View(hbll_tidy)

hbll_tidy <- gfplot:::scale_survey_predictors(hbll_tidy)

pg <- make_prediction_grid(hbll_tidy, survey = NULL, draw_boundary = FALSE
)$grid 

pg <- mutate(pg, combined = depth)

plot_survey_sets(pg, hbll_tidy, fill_column = "combined",
  fill_scale =
