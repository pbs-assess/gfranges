...
#browser()
  ...
  # control_min <- quantile(alldata$start_density[alldata$treat=="0"], 0.05)
  ...
  # 
  # if (order == "increasing") 
  alldata <- alldata[(order(alldata$start_density)), ] 
  
  #browser()
  
  alldata$depth2 <- alldata$depth^2
  
  #fit_formula <- paste0("treat ~ start_density") 
  #fit_formula <- paste0("treat ~ start_density + depth") 
  fit_formula <- paste0("treat ~ start_density + depth + trawled + muddy + any_rock + omega_s + x + y") 
 
  
  # need to be standardized? seems not
  # alldata$start_density.st <- (arm::rescale(alldata$start_density))*10 
  # alldata$depth.st <- arm::rescale(alldata$depth) 
  ...
  # alldata$trawled.st <- arm::rescale(alldata$trawled) 
  # fit_formula <- paste0("treat ~ start_density.st + depth.st + trawled.st + muddy.st + any_rock.st") #+ + omega_s
  
  
  #fit <- McSpatial::matchprop(cell_type ~ start_density + depth, data= alldata, ytreat = "source")
  ...
  # matched <- MatchIt::matchit(as.formula(fit_formula), data = alldata, method="nearest", discard="both")
  # mdata <- MatchIt::match.data(matched)
  
  # Uses logit matching for which most matches are higher that treatment values
  # mdata <- McSpatial::matchprop(as.formula(fit_formula), data = alldata, ytreat = 1, discard = "both", m.order = "decreasing")
  
  # Uses n mahalanobis distances
  mdata <- McSpatial::matchprop(as.formula(fit_formula), data = alldata, ytreat = 1, distance="probit", discard = "both", reestimate=TRUE) #m.order = "random", 
  #mdata <- McSpatial::matchmahal(as.formula(fit_formula), data = alldata, ytreat = 1, distance="mahal", discard = "both") #, reestimate=TRUE)
  #mdata <- McSpatial::matchmahal(as.formula(fit_formula), data = alldata, ytreat = 1, distance="probit", discard = "both") #, reestimate=TRUE) , m.order = "random"
...
```{r}
for (j in (variable_names)) {
  baci_formula <- paste0("log_density~after + cell_type + after:cell_type + distance + distance2 + std_", j, "+ std_", j, "2 + (1|icell) + (1|matchobs)")
}
...
```{r}
sjPlot::plot_model(m, type = "std2", order.terms=c(1,2,7,5,6,3,4), title = paste(species, "adult"))
```
...
for (j in (variable_names)) {
  gamm_formula <- paste0("log_density ~ time * cell_type + s(distance) + s(std_", j, ") + s(X,Y)")
}

gam4 <- gamm4::gamm4(as.formula(gamm_formula), random = ~(1|matchobs) + (1|origobs), data=data) # + s(X,Y, bs="re")

# for (j in (variable_names)) {
#   gamm_formula <- paste0("log_density ~ time * cell_type + s(distance) + s(std_", j, ") + te(X,Y)")
# }
...
idata$distance2 <- idata$distance^2
...
Misc exploring gamm 
```{r eval=FALSE}
# mi <- mgcv::gamm(log_density~ as.factor(after) + cell_type + after:cell_type + std_temp + std_temp2 + distance + distance2 + s(as.factor(idata$icell), bs="re") + s(as.factor(idata$matchobs), bs="re"), data=idata)
# summary(mi)
# 
# mi <- mgcv::gamm(log_density~ as.factor(after) * cell_type + s(std_temp) + s(distance) + s(as.factor(idata$matchobs), bs="re") , data=idata) # + s(X,Y, bs="re")
# summary(mi)
# mi
# 
# 
...
# idata$matchobs <- as.factor(idata$matchobs)
# 
# m4b <- gamm4::gamm4(log_density~ after * cell_type + s(std_temp) + s(distance) + #s(X) + s(Y) + 
