...
ggplot(data, aes(y=log_density, x=temp, colour=as.factor(after))) +
...
```{r}
m <- lmerTest::lmer(log_density~after + cell_type + after:cell_type + std_temp + std_temp2 + (1|matchobs), data=data)
summary(m)
```


```{r}
sjPlot::plot_model(m, title = paste(species, "adult"))
```
...
ggplot(idata, aes(y=log_density, x=temp, colour=as.factor(after))) +
  geom_point(alpha=0.35) +
  scale_colour_viridis_d(option="C") 
...
```{r}
mi <- lmerTest::lmer(log_density~after + cell_type + after:cell_type + std_temp + std_temp2 + (1|matchobs), data=idata)
summary(mi)
```

```{r}
sjPlot::plot_model(mi, title = paste(species, "immature"))
```
