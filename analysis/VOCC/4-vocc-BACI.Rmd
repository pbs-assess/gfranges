...
#browser()
  ...
  fit_formula <- paste0("cell_type ~ start_density + depth") 
 # fit_formula <- paste0("cell_type ~ start_density + depth + trawled + muddy + any_rock + omega_s") 
  
  for (j in (variable_names)) {
  fit_formula <- paste0("", fit_formula, " + ", j, "_s")
  }
  
  matched <- McSpatial::matchprop(as.formula(fit_formula), data = alldata, ytreat = "source")
  
  matched_time0 <- matched %>% mutate(after = 0, density = start_density) #%>% dplyr::select(-start_density, -end_density)
  matched_time1 <- matched %>% mutate(after = 1, density = end_density) #%>% dplyr::select(-start_density, -end_density)
...
Plot data
```{r}
ggplot(data, aes(y=log_density, x=temp, colour=time)) +
...
m <- lmerTest::lmer(log_density~after + cell_type + after:cell_type + std_temp + std_temp2 + distance + distance2 + (1|icell) + (1|matchobs), data=data)
summary(m)
```
...
GAMM
```{r}
gam4 <- gamm4::gamm4(log_density~ time * cell_type + s(std_temp) + s(distance) + s(X,Y), 
  random = ~(1|matchobs) + (1|origobs), data=data) # + s(X,Y, bs="re")
...
Plot data
```{r}
ggplot(idata, aes(y=log_density, x=temp, colour=time)) +
  geom_point(alpha=0.35) +
  scale_colour_viridis_d(option="C") 
...
Basic BACI
```{r}
mi <- lmerTest::lmer(log_density ~ as.factor(after) + cell_type + after:cell_type + std_temp + std_temp2 + distance + distance2 + (1|icell) + (1|matchobs), data=idata)
summary(mi)
```
...
GAMM
```{r}
igam4 <- gamm4::gamm4(log_density~ time * cell_type + s(std_temp) + s(distance) + s(X,Y), 
  random = ~(1|matchobs) + (1|origobs), data=idata) # + s(X,Y, bs="re")
