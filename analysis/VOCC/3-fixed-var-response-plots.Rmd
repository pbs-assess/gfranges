...
for (i in seq_len(length(temp_model_list))) {
  ...
  if (length(td[[i]])>0) td[[i]]$optimal_temp <- get_optimal_value(td[[i]], xlimits = c(0, 12))
}
...
tdat <- do.call("rbind", td) %>%
  ...
  mutate(age = ifelse(grepl("imm", model), "imm", "adult"),
    t_maxy = max(y_hat)) %>% 
  select(model, age, optimal_temp, t_maxy) %>%
  distinct() 
...
for (i in seq_len(length(model_list))) {
 ...
 if (length(do[[i]])>0) do[[i]]$optimal_do <- get_optimal_value(do[[i]], xlimits = c(0.5, 6))
}
...
odat <- do.call("rbind", do) %>%
  ...
  mutate(age = ifelse(grepl("imm", model), "imm", "adult"),
    do_maxy = max(y_hat)) %>% 
  select(model, age, optimal_do, do_maxy) %>%
  distinct()

params <- full_join(tdat, odat, by=c("model", "age"))
#params <- tdat
params$species <- species
#params$date <- Sys.Date()


other_params <- readRDS(paste0("data/optimal-values-by-species-by-age.rds"))
params <- rbind(other_params, params)
saveRDS(params, file = "data/optimal-values-by-species-by-age.rds")
...
do_plots <- list()
for (i in seq_len(length(model_list))) {
  d[[i]] <- fixed_density(model_list[[i]], predictor = "do_mlpl")
  if (length(d[[i]]) == 0) {
