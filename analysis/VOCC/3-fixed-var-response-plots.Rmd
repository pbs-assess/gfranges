...
params: 
    ...
    species: "Arrowtooth Flounder" #"Curlfin Sole"
    priors: FALSE # call model that used priors?
    log_temp: TRUE # should temperature be log()
    model_w_do: TRUE
    # covs is a covariate string used to name model
    # if
    #fix_depth: FALSE 
    #covs: "-tv-depth-all-"
    #covs: "-tv-depth-past-2015" 
    # or if 
    fix_depth: TRUE
    covs: "-fixed-all-" 
    #covs: "-fixed-past-2017"
    cov_number: 2 
---
...
try({
  ...
  try({adult <- readRDS(paste0("data/", spp,
    "/mod-mat-biomass-", spp, add2016, covs, "-", ssid_string, "-prior-", priors, ".rds"
  ))
  adult<-sdmTMB:::update_model(adult) 
  imm <- readRDS(paste0("data/", spp,
    "/mod-imm-biomass-", spp, add2016, covs, "-", ssid_string, "-prior-", priors, ".rds"
  ))
  imm <-sdmTMB:::update_model(imm) 
  })
  try({total <- readRDS(paste0("data/", spp,
    "/model-total-biomass-", spp, add2016, covs, "-", ssid_string, "-prior-", priors, ".rds"))
  })
})
...
for (i in seq_len(length(model_list))) {
 ...
 if(model_list[[i]]$model$convergence==0){
  ...
  if (length(do[[i]])>0) {
   ...
   do[[i]]$x <- exp(do[[i]]$x)
   do_plots[[i]] <- 
     plot_mountains(do[[i]], variable_label = "Dissolved O2", xlimits = c(0.65, 6.25), x_breaks = c(1,2,3,4,5,6,7,8)) +
...
for (i in seq_len(length(model_list))) {
 if(model_list[[i]]$model$convergence==0){
  ...
  if (length(td[[i]])>0) {
   ...
   temp_plots[[i]] <- 
     plot_mountains(td[[i]], variable_label = "Temperature", xlimits = c(4.68, 9.93), x_breaks = c(3,4,5,6,7,8,9,10,11)) +
        ggtitle(paste(species, names(model_list[i]))) + 
