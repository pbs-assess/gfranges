...
do <- do + ggtitle("Mean DO 2008-2012")
do
```


```{r}
temp <- plot_vocc(out0.5d,
  fill_col = "2_s",
  fill_label = "°C ",
  raster_alpha = 1,
  vec_aes = NULL,
  viridis_option = "C",
  raster_limits = c(4, 8.5)
  )

temp <- temp + ggtitle("Mean temperature 2008-2012")
temp
```
...
gvocc <- gvocc + ggtitle("VOCC <0.2 ml/L decline (excludes 2016)")
gvocc
```

```{r}
out0.5t <- make_vector_data(predicted,
...
out0.5t$temp.units_per_decade[out0.5t$temp.units_per_decade < 0.1] <- out0.5t$temp.units_per_decade[out0.5t$temp.units_per_decade < 0.1]-0.1
View(out0.5t)
...
vocct <- plot_vocc(out0.5t,
  ...
  raster_alpha = 1,
  min_vec_plotted = 5,
  raster_limits = c(-0.1, 1.5))
vocct <- vocct + ggtitle("VOCC <0.5 °C increase (excludes 2016)")
vocct
```
