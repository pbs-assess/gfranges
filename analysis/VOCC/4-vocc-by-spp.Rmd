...
climate_string <- gsub(" ", "-", gsub("\\/", "-", tolower(climate_string)))
```
...
plot <- gridExtra::grid.arrange(
  ...
  ncol = 2,
  top = grid::textGrob(paste(climate, "-", optimal_value, "-",
    min_string,"to", max_string, "-", start_time, "-", delta_t_total, ".rds"))
) 
...
if (length(variable_names) == 1) {
  ...
  vectors <- vectors %>% filter( paste0("X",j) < (optimal_value[j] - min_thresholds[j]) )
  }
