...
vocc_plot <- gridExtra::grid.arrange(
  ...
  ncol = 2,
  top = grid::textGrob(paste("do of ", optimal_value[1], "less", min_thresholds[1],"or temperature of ", optimal_value[2], "plus", max_thresholds[2], " over ", start_time, "-", start_time + delta_t_total, ""))
) 
...
#       initial_value = UQ(rlang::sym(paste0(variable_names, "_s"))), #var_1_s, 
#       source_end_value = UQ(rlang::sym(paste0(variable_names, "_e"))), #var_1_e, 
...
#   
#   if (climate == "do") vectors <- vectors %>% 
#       filter(source_end_value < (optimal_do - do_threshold))
#   if (climate == "temperature") vectors <- vectors %>% 
#       filter(source_end_value > (optimal_temp + temp_threshold))
# 
# } else { # number of variables > 1
  # 
  # vectors <- vocc %>%
  #   mutate(
  ...
  #     km_per_decade, timespan) 
  # 
  # # split source_end_value into multiple vectors
  # source_end <- purrr::map_df(strsplit(vectors$source_end_value, " "), 
  #                  function(x) data.frame(t(as.numeric(x))))
  # 
  # vectors <- cbind(vectors, source_end) 
  # 
  # for (j in seq_along(variable_names)){
  #   
  #   source_end_climatej <- paste0("X",j)
  #   vectors <- vectors %>% filter( 
  #     (!!sym(source_end_climatej)) < (optimal_value[j] + critical_thresholds[j])
  #     )
  # }
#  
#   #climate <- "do and temperature"
  ...
  vectors <- vocc %>%
    mutate(
      ...
      if (min_thresholds[j] != Inf) { 
        vectors <- filter(vectors, UQ(rlang::sym(paste0(variable_names[j], "_s"))) < optimal_value[j] - min_thresholds[j]) 
        }
      if (max_thresholds[j] != Inf) {
        vectors <- filter(vectors, UQ(rlang::sym(paste0(variable_names[j], "_s"))) > optimal_value[j] + max_thresholds[j])
        }
  
  #FIXME: decide which variables might cause problems in future merges and remove them...   
  #   vectors <- vectors %>% select(-N) 
  # 
...
if (length(vectors[ ,2]) == 0) {
  try({other_coefs <- readRDS("data/vocc-by-spp-sdmTMB-coefs.RDS")})
  coefs <- other_coefs[1,]
  coefs$var <- "NA"
  coefs$beta <- "NA"
  coefs$se <- "NA"
  ...
  rbind(other_coefs, coefs)
  saveRDS(coefs, file = "data/vocc-by-spp-sdmTMB-coefs.RDS")
...
vocc_by_sp[[i]] <- rbind(vector_source, vector_target) %>% 
  filter(!is.na(icell)) %>% 
# remove potential source cells with no fish  
  #FIXME: should their be a flexible min density of fish for a source cell?
  filter(source_density > 0.000001) 
...
# calculate standardized density varibles
vocc_by_sp[[i]] <- vocc_by_sp[[i]] %>% 
  mutate(
    perc_change = (end_density - start_density)/start_density, 
    cell_type = as.factor(cell_type),
    #std_change = (perc_change)/sd.change, 
...
# set distance intercept at cell size
    distance = (ifelse(distance > 100, 100, distance)) - input_cell_size, 
...
vocc_by_sp[[i]] <- vocc_by_sp[[i]] %>% 
  mutate(std_change = (perc_change)/sd.change)
...
vocc_by_sp[[i]] <- vocc_by_sp[[i]] %>% rename(
  source_end_climateA = X1, source_end_climateB = X2
  )

#browser()

icells <- vocc_by_sp[[i]] %>% distinct(icell, .keep_all = TRUE)
mean.climateA <- mean(icells$source_end_climateA, na.rm = TRUE)
sd.climateA <- sd(icells$source_end_climateA, na.rm = TRUE)
mean.climateB <- mean(icells$source_end_climateB, na.rm = TRUE)
sd.climateB <- sd(icells$source_end_climateB, na.rm = TRUE)

vocc_by_sp[[i]] <- vocc_by_sp[[i]] %>% 
  mutate(
    mean_climateA = mean.climateA, sd_climate = sd.climateA,
    std_end_climateA = (source_end_climateA - mean.climateA)/(sd.climateA*2),
    std_end_climateA2 = std_end_climateA^2,
    mean_climateB = mean.climateB, sd_climate = sd.climateB,
    std_end_climateB = (source_end_climateB - mean.climateB)/(sd.climateB*2),
    std_end_climateB2 = std_end_climateB^2
    )

#browser()
  end_climate <- purrr::map_df(strsplit(vocc_by_sp[[i]]$end_climate, " "), 
                   function(x) data.frame(t(as.numeric(x))))
  
  vocc_by_sp[[i]] <- cbind(vocc_by_sp[[i]], end_climate) %>%
    rename(end_climate_chr = end_climate, end_climate = X1, end_climateB = X2 )

} else {
  
 stop("Doesn't currently support testing vectors based on more than two climate variables at once.")
}
}
...
mature_plot2 <- ggplot(data, aes(x = distance, y = std_change, 
...
# std_max <- quantile(data$std_change, 0.9995)
# 
...
# Xmax <- quantile(idata$X, 0.9995)
# Xmin <- quantile(idata$X, 0.005)
# std_max <- quantile(idata$std_change, 0.9995)
# 
# idata <- idata %>% filter ( X >= Xmin & X <= Xmax )
#  
...
ixy <- as.data.frame(cbind(idata$X, idata$Y))
...
if (n_knots> nrow(unique(xy))) { n_knots <- nrow(unique(ixy))}
