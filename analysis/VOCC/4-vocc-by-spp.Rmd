...
Get most up to date optimal values from mountain plots for most abundant year
...
rm(vocc)
try({
  vocc <- readRDS(paste0("data/vocc-", ssid_string,"-", climate_string, "-",
...
saveRDS(vocc, file = paste0("data/vocc-", ssid_string,"-", climate_string, "-",
    min_string,"to", max_string, "-", start_time, "-", delta_t_total, ".rds"))
}
# glimpse(vocc)
```

### Plot all climate vectors
```{r eval = FALSE}
...
vectors <- vectors %>%
      mutate(
        ...
        distance = ifelse(distance>120, 120, distance),
...
vocc_by_sp <- rbind(vector_source, vector_target) %>% 
  ...
  filter(!is.na(icell)) %>% 
  filter(!is.na(source_density)) %>% filter(!is.na(end_density)) %>% 
...
coefs <- rbind(other_coefs, coefs) %>% distinct() %>% 
 mutate_if(is.numeric, funs(signif(.,digits = 2)))
}

coefs <- coefs %>% distinct()
...
ggplot(data, aes(x = UQ(rlang::sym(paste0("end_", j))), y = (end_density), 
...
try({other_coefs <- readRDS(paste0("data/vocc-", climate_string, "-vocc-by-spp-sdmTMB-coefs.RDS"))})
...
beta_target <- lmer_coefs %>% filter(ssid_string == "1n3") %>% filter(var == "cell_typetarget")
