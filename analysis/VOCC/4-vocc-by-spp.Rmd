...
if (length(vectors[ ,2]) == 0) {
  other_coefs <- readRDS("data/vocc-by-spp-sdmTMB-coefs.RDS")
  coefs <- other_coefs[1,]
  coefs$var <- "NA"
  coefs$beta <- "NA"
  coefs$se <- "NA"
...
vector_source <- left_join(vectors, start, by = c( "icell", "x", "y")) %>% 
  ...
  select(start_density, end_density, target_X, target_Y)
...
vocc_by_sp <- mutate(vocc_by_sp,
  ...
  (!! std.var) := (UQ(rlang::sym(paste0(variable_names[j], "_e"))) - icells[1, mean.var])/ icells[1, sd.var],
...
ggplot(data, aes(x = UQ(rlang::sym(paste0("end_", j))), y = (end_density # - start_density
  ...
  xlab(paste("Final value of", variable_names,"for all cells")) +
