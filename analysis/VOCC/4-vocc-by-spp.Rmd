...
if (length(variable_names) == 1) {
  vectors <- vocc %>%
    mutate(
      ...
      initial_value = var_1_s, 
      source_end_value = var_1_e, 
  ...
  source_end <- purrr::map_df(strsplit(vectors$source_end_value, " "), function(x) data.frame(t(as.numeric(x))))
  vectors <- cbind(vectors, source_end) 
  
  for (j in seq_along(variable_names)){
  browser()
    vectors <-  filter( vectors, paste0("X",j) < (optimal_value[j] - min_thresholds[j]) )
  }
...
vocc_by_sp[[i]] <- vocc_by_sp[[i]] %>% 
 ...
 stop("Doesn't currently support testing vectors bases on more than two climate variables at once.")
}
}
