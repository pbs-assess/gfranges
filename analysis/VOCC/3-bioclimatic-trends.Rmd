...
if (any(names(maturity) == "adult_density")) {
  ...
  # nd_pre_2011 <- nd %>% filter(year < 2011)
  # ad_model_2011 <- readRDS(paste0("data/", spp,
  #       "/mod-mat-biomass-", spp, covs,"-past-2011-", ssid_string, "-prior-", priors, ".rds"))
  # sort(unique(ad_model_2011$data$year))
  # adult_pre_2011 <- predict(ad_model_2011, newdata = nd_pre_2011, return_tmb_object = TRUE)
  # 
  # fake_2011 <- nd %>%
  #   mutate(year = year - 2) %>%
  #   filter(year > 2007) %>% filter(year < 2011 )
  # sort(unique(fake_2011$year))
  # sort(unique(nd_pre_2011$year))
  # adult_2011 <- predict(ad_model_2011, newdata = fake_2011, return_tmb_object = TRUE)
  # adult_2011$data$year <- adult_2011$data$year + 2

  
  nd_pre_2013 <- nd %>% filter(year < 2013)
  ad_model_2013 <- readRDS(paste0("data/", spp, 
        "/mod-mat-biomass-", spp, covs,"-past-2013-", ssid_string, "-prior-", priors, ".rds"))
  adult_pre_2013 <- predict(ad_model_2013, newdata = nd_pre_2013, return_tmb_object = TRUE)
  
  fake_2013 <- nd %>% 
    mutate(year = year - 2) %>% 
    filter(year > 2007) %>% filter(year < 2013 )
  sort(unique(fake_2013$year))
  sort(unique(nd_pre_2013$year))
  adult_2013 <- predict(ad_model_2013, newdata = fake_2013, return_tmb_object = TRUE)
  adult_2013$data$year <- adult_2013$data$year + 2 
  
  nd_pre_2015 <- nd %>% filter(year < 2015)
  ad_model_2015 <- readRDS(paste0("data/", spp, 
        "/mod-mat-biomass-", spp, covs,"-past-2015-", ssid_string, "-prior-", priors, ".rds"))
  adult_pre_2015 <- predict(ad_model_2015, newdata = nd_pre_2015, return_tmb_object = TRUE)
  
  fake_2015 <- nd %>% 
    mutate(year = year - 2) %>% 
    filter(year > 2007) %>% filter(year < 2015)
  sort(unique(nd_pre_2015$year))
  sort(unique(fake_2015$year))
  adult_2015 <- predict(ad_model_2015, newdata = fake_2015, return_tmb_object = TRUE)
  adult_2015$data$year <- adult_2015$data$year + 2 
  
  nd_pre_2017 <- nd %>% filter(year < 2017)
  ad_model_2017 <- readRDS(paste0("data/", spp, 
        "/mod-mat-biomass-", spp, covs,"-past-2017-", ssid_string, "-prior-", priors, ".rds"))
  adult_pre_2017 <- predict(ad_model_2017, newdata = nd_pre_2017, return_tmb_object = TRUE)
  
  fake_2017 <- nd %>% 
    mutate(year = year - 2) %>% 
    filter(year > 2007) #%>% filter(year < 2013 )
  sort(unique(fake_2017$year))
  sort(unique(nd_pre_2017$year))
   
  adult_2017 <- predict(ad_model_2017, newdata = fake_2017, return_tmb_object = TRUE)
  adult_2017$data$year <- adult_2017$data$year + 2 

  fake_2017_minus4 <- nd %>% 
    mutate(year = year - 4) %>% 
    filter(year > 2007)
  sort(unique(fake_2017_minus4$year))
  sort(unique(nd_pre_2015$year))
  adult_2017_multi <- predict(ad_model_2015, newdata = fake_2017_minus4, return_tmb_object = TRUE)
  adult_2017_multi$data$year <- adult_2017_multi$data$year + 4
  sort(unique(adult_2017_multi$data$year))
   
  past_predictions <- adult_pre_2015$data %>% 
   mutate(past_est = est, blob_bioclim = est) %>% 
   select(X, Y, year, past_est, blob_bioclim) 

  # bioclim_2011_start <- filter(adult_pre_2011$data, year >= 2009) %>% filter(year <= 2010) %>%
  #   mutate(bioclim_2011 = est)
  # bioclim_2011_end <- filter(adult_2013$data, year >= 2011) %>% filter(year <= 2012) %>%
  #   mutate(bioclim_2011 = est_non_rf + omega_s)
  # bioclim_2011_all <- dplyr::bind_rows(bioclim_2011_start, bioclim_2011_end) 

 
  bioclim_2013_start <- filter(adult_pre_2013$data, year >= 2011) %>% filter(year <= 2012) %>%
    mutate(bioclim_2013 = est) #%>% select(X,Y, year, bioclim_2013)
  # bioclim_2013_all <- left_join(bioclim_2011, bioclim_2013_start) %>% 
  #   select(X,Y, year, bioclim_2011, bioclim_2013)
  #past_predictions <- left_join(past_predictions, bioclim_2011_all) 

  
  bioclim_2013_end <- filter(adult_2013$data, year >= 2013) %>% filter(year <= 2014) %>%
    mutate(bioclim_2013 = est_non_rf + omega_s)
  
  bioclim_2013_all <- dplyr::bind_rows(bioclim_2013_start, bioclim_2013_end) 
  
  bioclim_2015_start <- filter(adult_2015$data, year >= 2013) %>% filter(year <= 2014) %>%
    mutate(bioclim_2015 = est) %>% 
    select(X,Y, year, bioclim_2015)
  
  bioclim_2013_all <- left_join(bioclim_2013_all, bioclim_2015_start) %>% 
    select(X,Y, year, bioclim_2013, bioclim_2015)
  past_predictions <- left_join(past_predictions, bioclim_2013_all) 
  
  
  bioclim_2015_end <- filter(adult_2015$data, year >= 2015) %>% filter(year <= 2016) %>%
    mutate(blob_bioclim = est_non_rf + omega_s, bioclim_2015 = est_non_rf + omega_s) 
  
  bioclim_2017_start <- filter(adult_2017$data, year >= 2015) %>% filter(year <= 2016) %>%
    mutate(bioclim_2017 = est) %>% select(X,Y, year, bioclim_2017)
  
  bioclim_2015 <- left_join(bioclim_2015_end, bioclim_2017_start) 
  
  bioclim_2017_multi <- filter(adult_2017_multi$data, year >= 2017) %>% filter(year <= 2018) %>%
    mutate(blob_bioclim = est_non_rf + omega_s) %>% 
    select(X,Y, year, blob_bioclim)
  
  bioclim_2017 <- filter(adult_2017$data, year >= 2017) %>% filter(year <= 2018) %>%
    mutate(bioclim_2017 = est_non_rf + omega_s)
  bioclim_2017 <- left_join(bioclim_2017, bioclim_2017_multi) 
  
  adult_predictions <- adult_predictions_obj$data
  adult_bioclim <- dplyr::bind_rows(past_predictions, bioclim_2015, bioclim_2017) %>% 
    select(X, Y, year, past_est, blob_bioclim, bioclim_2013, bioclim_2015, bioclim_2017)

  adult_predictions <- left_join(adult_predictions, adult_bioclim)
  ...
  nd_pre_2013 <- nd %>% filter(year < 2013)
  im_model_2013 <- readRDS(paste0("data/", spp, 
        "/mod-imm-biomass-", spp,"-fixed-past-2013-", ssid_string, "-prior-", priors, ".rds"))
  imm_pre_2013 <- predict(im_model_2013, newdata = nd_pre_2013, return_tmb_object = TRUE)
  ...
  sort(unique(fake_2013$year))
  sort(unique(nd_pre_2013$year))
  imm_2013 <- predict(im_model_2013, newdata = fake_2013, return_tmb_object = TRUE)
  imm_2013$data$year <- imm_2013$data$year + 2 
  
  nd_pre_2015 <- nd %>% filter(year < 2015)
  im_model_2015 <- readRDS(paste0("data/", spp, 
        "/mod-imm-biomass-", spp,"-fixed-past-2015-", ssid_string, "-prior-", priors, ".rds"))
  imm_pre_2015 <- predict(im_model_2015, newdata = nd_pre_2015, return_tmb_object = TRUE)
  ...
  sort(unique(nd_pre_2015$year))
  sort(unique(fake_2015$year))
  imm_2015 <- predict(im_model_2015, newdata = fake_2015, return_tmb_object = TRUE)
  imm_2015$data$year <- imm_2015$data$year + 2 
  
  nd_pre_2017 <- nd %>% filter(year < 2017)
  im_model_2017 <- readRDS(paste0("data/", spp, 
        "/mod-imm-biomass-", spp,"-fixed-past-2017-", ssid_string, "-prior-", priors, ".rds"))
  imm_pre_2017 <- predict(im_model_2017, newdata = nd_pre_2017, return_tmb_object = TRUE)
  ...
  imm_2017 <- predict(im_model_2017, newdata = fake_2017, return_tmb_object = TRUE)
  imm_2017$data$year <- imm_2017$data$year + 2 

  fake_2017_minus4 <- nd %>% 
    mutate(year = year - 4) %>% 
    filter(year > 2007)
  sort(unique(fake_2017_minus4$year))
  imm_2017_multi <- predict(im_model_past, newdata = fake_2017_minus4, return_tmb_object = TRUE)
  imm_2017_multi$data$year <- imm_2017_multi$data$year + 4
  sort(unique(imm_2017_multi$data$year))
   
 past_predictions <- imm_pre_2015$data %>% 
   mutate(past_est = est, blob_bioclim = est) %>% 
   select(X, Y, year, past_est, blob_bioclim) 
  
  bioclim_2013_start <- filter(imm_pre_2013$data, year >= 2011) %>% filter(year <= 2012) %>%
    mutate(bioclim_2013 = est)
  bioclim_2013_end <- filter(imm_2013$data, year >= 2013) %>% filter(year <= 2014) %>%
    mutate(bioclim_2013 = est_non_rf + omega_s)
  bioclim_2013 <- dplyr::bind_rows(bioclim_2013_start, bioclim_2013_end) 
  
  bioclim_2015_start <- filter(imm_past$data, year >= 2013) %>% filter(year <= 2014) %>%
    mutate(bioclim_2015 = est) %>% select(X,Y, year, bioclim_2015)
  bioclim_2013 <- left_join(bioclim_2013, bioclim_2015_start) %>% 
    select(X,Y, year, bioclim_2013, bioclim_2015)
  past_predictions <- left_join(past_predictions, bioclim_2013) 
  
  bioclim_2015 <- filter(imm_2015$data, year >= 2015) %>% filter(year <= 2016) %>%
    mutate(blob_bioclim = est_non_rf + omega_s, bioclim_2015 = est_non_rf + omega_s) 
  bioclim_2017_start <- filter(imm_2017$data, year >= 2015) %>% filter(year <= 2016) %>%
    mutate(bioclim_2017 = est) %>% select(X,Y, year, bioclim_2017)
  
  bioclim_2015 <- left_join(bioclim_2015, bioclim_2017_start) 
  
  bioclim_2017_multi <- filter(imm_2017_multi$data, year >= 2017) %>% filter(year <= 2018) %>%
    mutate(blob_bioclim = est_non_rf + omega_s) %>% 
    select(X,Y, year, blob_bioclim)
  
  
  bioclim_2017 <- filter(imm_2017$data, year >= 2017) %>% filter(year <= 2018) %>%
    mutate(bioclim_2017 = est_non_rf + omega_s)
  bioclim_2017 <- left_join(bioclim_2017, bioclim_2017_multi) 
  
   imm_predictions <- imm_predictions_obj$data
   imm_bioclim <- dplyr::bind_rows(past_predictions, bioclim_2015, bioclim_2017) %>% 
    select(X, Y, year, past_est, blob_bioclim, bioclim_2013, bioclim_2015, bioclim_2017)

  imm_predictions <- left_join(imm_predictions, imm_bioclim)
...
# }
 ...
 if (exists("adult_predictions")) {
  ...
  p_adult_st <- list()
  for (i in seq_len(length(model_ssid))) {
    p_adult_st[[i]] <-plot_facet_map(ad_prediction_list[[i]], "epsilon_st",
    white_zero = TRUE,
    high_fill = "#66C2A5",
    low_fill = "#5E4FA2"
    #raster_limits = c(0, max_raster),
...
data <- mutate(adult_predictions, cell_id = paste0(X,Y)) %>% 
  group_by(cell_id) %>% 
  mutate(cell_max = max(bioclimatic)) %>% group_by(year.f) %>% mutate(ann_biomass = sum(est_exp))
...
library(forcats)
violin_data <- data %>% filter (cell_max > threshold) %>% mutate(year = fct_reorder(year.f, desc(ann_biomass)))
 glimpse(violin_data)
