...
if (any(names(maturity) == "adult_density")) {
  ...
  # write as function
  bioclim_by_year <- function(yr, model_for_prior_year, pred_grid) {
    nd <- pred_grid
    ...
    prior_years <- predict(model_for_prior_year, newdata = nd_trim, return_tmb_object = FALSE)
    fake_est <- predict(model_for_prior_year, newdata = fake_years, return_tmb_object = FALSE)
  ...
  # for multiyear bioclim
  bioclim_multi_year <- function(yr, model_for_prior_year, pred_grid) {
    nd <- pred_grid
    ...
    fake <- sort(unique(fake_years$year))
    true <- sort(unique(model_for_prior_year$data$year))
    ...
    if (period2_range >= 4) {
      ...
      fake_est_minus4 <- predict(model_for_prior_year, newdata = fake_minus4, return_tmb_object = FALSE)
    ...
    if (period2_range >= 6) {
      ...
      fake_est_minus6 <- predict(model_for_prior_year, newdata = fake_minus6, return_tmb_object = FALSE)
