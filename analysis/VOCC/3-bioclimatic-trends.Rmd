...
if (any(names(maturity) == "adult_density")) {
  ...
  adult_predictions$est_rw_i <- adult_predictions_obj$obj$report()$proj_rw_i
  # test_eta <- adult_predictions_obj$obj$report()$proj_eta
  # test_fe <- adult_predictions_obj$obj$report()$proj_fe
  # 
  # test_diff <- test_eta - test_fe
  # range(adult_predictions$est_rw_i)
  # report <- adult_predictions_obj$obj$report()
  
  saveRDS(adult_predictions, file = paste0("data/", spp,
    "/predictions-", spp, covs, "-", ssid_string, "-mature-bioclimatic-prior-", priors, ".rds"))
  ...
  imm_predictions$est_rw_i <- imm_predictions_obj$obj$report()$proj_rw_i
  
  
  saveRDS(imm_predictions, file = paste0("data/", spp,
    "/predictions-", spp, covs, "-", ssid_string, "-imm-bioclimatic-prior-", priors, ".rds"))
...
if (region != "All synoptic surveys") {
  try(adult_predictions <- readRDS(paste0("data/", spp,
    "/predictions-", spp, covs, "-1n3n4n16-mature-bioclimatic-prior-", priors, ".rds"
  )) %>% filter(ssid %in% model_ssid))
  try(imm_predictions <- readRDS(paste0("data/", spp,
    "/predictions-", spp, covs, "-1n3n4n16-imm-bioclimatic-prior-", priors, ".rds"
  )) %>% filter(ssid %in% model_ssid))
  ...
  if (bind_predictions) {
    try({pred1n3 <- readRDS(paste0("data/", spp,
      "/predictions-", spp, covs, "-1n3-mature-bioclimatic-prior-", priors, ".rds"
      ))
    })
    try({pred4 <- readRDS(paste0("data/", spp,
      "/predictions-", spp, covs, "-4-mature-bioclimatic-prior-", priors, ".rds"
      ))
    })
    try({pred16 <- readRDS(paste0("data/", spp,
      "/predictions-", spp, covs, "-16-mature-bioclimatic-prior-", priors, ".rds"
      ))
    })
    ...
    try({pred1n3_imm <- readRDS(paste0("data/", spp,
      "/predictions-", spp, covs, "-1n3-imm-bioclimatic-prior-", priors, ".rds"
      ))
    })
    try({pred4_imm <- readRDS(paste0("data/", spp,
      "/predictions-", spp, covs, "-4-imm-bioclimatic-prior-", priors, ".rds"))
    })
    try({pred16_imm <- readRDS(paste0("data/", spp,
        "/predictions-", spp, covs, "-16-imm-bioclimatic-prior-", priors, ".rds"))
    })
  ...
  if (!exists("adult_predictions")) {
      ...
      if (!exists("predictions")) {
        try({pred1n3_all <- readRDS(paste0("data/", spp,
          "/predictions-", spp, covs, "-1n3-total-bioclimatic-prior-", priors, ".rds"))
        })
        try({pred4_all <- readRDS(paste0("data/", spp,
          "/predictions-", spp, covs, "-4-total-bioclimatic-prior-", priors, ".rds"))
        })
        try({pred16_all <- readRDS(paste0("data/", spp,
          "/predictions-", spp, covs, "-16-total-bioclimatic-prior-", priors, ".rds"))
        })
        try({
...
if (exists("adult_predictions")) {
  ...
  adult_predictions$bioclimatic <- exp(adult_predictions$est - adult_predictions$epsilon_st) * 10000
  imm_predictions$bioclimatic <- exp(imm_predictions$est - imm_predictions$epsilon_st) * 10000
  ...
  predictions$bioclimatic <- exp(predictions$est - predictions$epsilon_st) * 10000
...
png(
  file = paste0("figs/", spp, "/bioclimatic-biomass-", spp, covs, "-", ssid_string, "-priors-", priors, ".png"),
  res = 600,
  units = "in",
  width = 16,
  height = 8.5
...
```{r results = "hide"}
png(
  file = paste0("figs/", spp, "/bioclimatic-exploratory-", spp, covs, "-", ssid_string, "-priors-", priors, ".png"),
  res = 600,
  units = "in",
  width = 9.5,
  height = 12.5
...
png(
  file = paste0("figs/", spp, "/bioclimatic-residuals-", spp, covs, "-", ssid_string, "-priors-", priors, ".png"),
  res = 600,
  units = "in",
  width = 16,
  height = 8.5
