...
```{r}
  if (any(names(data) == "adult_density")) {
    ...
    adult_formula <- as.formula(paste(
      "adult_density ~ 0 + as.factor(year) + do_mlpl_scaled + do_mlpl_scaled2 + depth_scaled + depth_scaled2", covariates, ""
      ))
    ...
    imm_formula <- as.formula(paste(
      "imm_density ~ 0 + as.factor(year) + do_mlpl_scaled + do_mlpl_scaled2 + depth_scaled + depth_scaled2", covariates, ""
      ))
    ...
    starttime3 <- Sys.time()
    dens_formula <- as.formula(paste("density ~ 0 + as.factor(year) + do_mlpl_scaled + do_mlpl_scaled2 + depth_scaled + depth_scaled2", covariates, ""))
