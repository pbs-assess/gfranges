...
d_all <- left_join(d_all, fishing, by = c("X", "Y", "year"))
d_all$log_effort[is.na(d_all$log_effort)] <- -2.3
...
write.csv(vocc, file = paste0(
  "data/_new_biotic/", age,  "-biotic-vocc-", spp, "-", ssid_string, "-", covs, "-", min(d$year), "-", max(d$year), "-trim10.csv"
))
```
