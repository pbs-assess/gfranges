...
if (immature) {
  ...
  "/predictions-", spp, covs, "-1n3n4n16-imm-biomass-prior-FALSE.rds")) %>% filter(year > 2007) 

  } else {
    age <- "mature" 
      try({
        d_all <-readRDS(paste0("data/", spp,
        "/predictions-", spp, covs, "-1n3n4n16-total-biomass-prior-FALSE.rds")) %>% filter(year > 2007) 
      })
      try({
        d_all <-readRDS(paste0("data/", spp,
...
write.csv(vocc, file = paste0(
  "data/biotic_test/biotic-vocc-", spp, "-", age, covs, "-", ssid_string, "-", min(d$year), "-", max(d$year), ".csv"
))
