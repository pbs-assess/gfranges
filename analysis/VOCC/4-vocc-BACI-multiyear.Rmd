...
Should optimal temperature vary for age classes? Seems likely. 
Therefore need separate VOCC trimming and matching for age classes -> change function structure to choose age upfront 
...
params <- params[rev(order(params$maxy)), ]
...
if (!exists("vocc_by_sp")){
match_plots <- lapply(matched, function(i){  
  summary(i)
  plot(i)
  })
match_plots
...
try({
  vocc_by_sp <- readRDS(paste0(
    "data/ BACI-", species, "-", ssid_string, "-", climate_string, "-",
    min_string, "to", max_string, "-multiyear-all.rds"
  ))
})
...
#   scale_colour_viridis_d(option="C") +
#     facet_grid(start_time~time) +
#   geom_text(data = idata_depth,
#     aes(150, 0.5, label = round(mean_depth)), hjust = -0.3, inherit.aes = FALSE) +
#   theme_bw()
ggplot(idata, aes(depth, fill = cell_type)) +
  geom_density(alpha=0.5) +
  scale_fill_viridis_d(option="C") +
  facet_wrap(~start_time) +
  theme_bw()
...
#   # "time * start_time + ",
#   "start_time * cell_type + ",
#   "time * cell_type + s(vect_dist) + s(std_", j, ") + s(X,Y)")
#  }
igam4 <- gamm4::gamm4(as.formula(gamm_formula), 
  random = ~(1|matchobs) + (1|icell) 
   + (1|start_time)
  # + (time|start_time) 
  # + (cell_type|start_time)
