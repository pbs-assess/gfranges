...
```{r}
alldata <- lapply(biomass_list, function(split_bio_data) {
  ...
  for (j in seq_along(start_years)) {
   ...
   biomass_by_year[[j]] <- make_trend_data(split_bio_data,
    ssid = model_ssid,
    start_time = start_time,
    skip_time = skip_time,
    end_time = end_time,
...
matched <- lapply(alldata, function(alldata) {  
 ...
 ## Using optimization ... doesn't work   
  ...
  fit_formula <- paste0("treat ~ depth.st + muddy.st + any_rock.st + 
    trawled + omega_s + x + y") #
  # for (j in (variable_names)) {
  #   fit_formula <- paste0("", fit_formula, " + ", j, "_s")
  #   }
