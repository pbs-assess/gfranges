...
```{r echo=TRUE}
global_only <- TRUE
biomass_threshold <- "75" 

# biomass_threshold <- "50" # filter for climate values predicted to support 50% of max biomass 
#  biomass_threshold <- "25" # filter for climate values predicted to support 25% of max biomass... doesn't work for DO 
...
if (global_only) {
 ...
 if (model_ssid == 4) {
   params <- params %>% mutate(survey4 = ifelse(grepl("4", model), "T", "F"))
   params <- filter(params, survey4 == "T")
 }
}
...
if (climate == "temperature") {
  ...
  upper_model <- params[rev(order(params$t_maxy)), ]$model
...
if (climate == "do") {
  ...
  params <- filter(params, lower_do_25 > 0.4)
  ...
  lower_model <- params[rev(order(params$do_maxy)), ]$model
  upper_thresholds <- NA
  upper_model <- NULL
...
png(
  file = paste0("figs/", spp,
    ...
    climate_string, "-perc_", biomass_threshold, "-", optimal_string, "-allpairs1.png"
  ),
  res = 600,
  units = "in",
  width = 8.5,
...
```{r eval=F}
 gamm_formula <- paste0("log_density ~ ", 
  ...
  "time * cell_type  + s(std_log_depth) + s(X,Y) ") #+ s(vect_dist)
