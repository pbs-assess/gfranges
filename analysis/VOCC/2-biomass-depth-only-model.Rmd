...
if (params$update_model) {
  ...
  # if updating model, will need new version of mesh to work with changes to sdmTMB
  ...
  if (any(names(data) == "adult_density")) {
   ...
   #    time = "year", spde = spde,
   #    family = tweedie(link = "log"),
   #    # ar1_fields = TRUE, # changed to FALSE July 2020
   #    # reml = TRUE,
   #    include_spatial = TRUE,
   #    nlminb_loops = 2, 
   #    newton_steps = 2,
   ...
   #  
    ...
    total_biomass <- sdmTMB::sdmTMB(
      ...
      time = "year", spde = spde,
      family = tweedie(link = "log"),
      # ar1_fields = TRUE, # all off in July 2020
      include_spatial = TRUE,
      nlminb_loops = 2, 
      newton_steps = 2,
      enable_priors = priors,
