...
params: 
    ...
    knots: 800
    covariates: "+trawled+as.factor(ssid) + trawled:as.factor(ssid)" 
    # additional fixed-effect covariates
    covs: "-trawled-ssid-reml" # covariate string used to name model rds 
    update_model: TRUE
---
...
if (any(names(data) == "adult_density")) {
  ...
  adult_biomass <- sdmTMB::sdmTMB(data=data,
    adult_formula,
  ...
  imm_biomass <- sdmTMB::sdmTMB(data=data,
    imm_formula,
    time_varying = ~ 0 + depth_scaled + depth_scaled2, #+ trawled
    time = "year", spde = spde,
    family = tweedie(link = "log"),
  ...
  total_biomass <- sdmTMB::sdmTMB(data=data,
    dens_formula,
    time_varying = ~ 0 + depth_scaled + depth_scaled2, # + trawled
    time = "year", spde = spde,
    family = tweedie(link = "log"),
...
saveRDS(point_predictions, file = paste0("data/", spp,
    "/check-mod-mat-biomass-", spp, covs, "-", ssid_string, "-prior-", priors, ".rds"))

```

```{r}
depth_only_predictions <- readRDS(paste0("data/", spp,
    "/check-mod-mat-biomass-", spp, covs, "-", ssid_string, "-prior-", priors, ".rds"))

depth_only_predictions <- filter(depth_only_predictions, ssid %in% c(1,3)) %>% filter (year > 2007)

ggplot(depth_only_predictions, aes(adult_density, residuals, colour = adult_density)) + 
  geom_point() + scale_x_continuous(trans = 'log10') + facet_wrap(~year)
```

```{r}
ggplot(depth_only_predictions, aes((est), residuals, colour = adult_density)) +
  geom_point() + 
