...
```{r}
  ...
  mutate(exclude = if_else(do_mlpl>8, 1, 0)) %>% filter(exclude != 1) %>% # added on july 24 after first round of models run...
  ...
  data <- do_data %>% mutate(depth = raw_depth) %>% 
    filter(ssid %in% model_ssid) %>% filter(!is.na(do_mlpl))# %>% filter(year != 2016)
```
...
```{r}
  if (any(names(data) == "adult_density")) {
    ...
    saveRDS(total_biomass_d, file = paste0("data/", spp, 
      "/model-total-biomass-", spp, "-do", covs, "-", ssid_string, "-prior-", priors, ".rds"
      ))
  }
```
