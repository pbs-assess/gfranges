...
```{r}
nd_all <- readRDS(paste0("data/predicted-DO-june-2020.rds")) %>% select(X, Y, year, temp, do_est)
...
gf3 <- gf3 %>%
  mutate(
    ...
    ## if default of divisor = 10
      trend_per_year = (trend/10), # percent change not raw
      trend_per_decade = trend_per_year * 10,
    velocity = velocity
  ) %>%
  dplyr::mutate(X = x, Y = y)
...
DO_values <- gf0 %>%
  rename(
    DO_vel = velocity,
    DO_dvocc = dvocc,
    DO_trend = trend_per_year, # should change to trend_per_decade
    DO_grad = gradient,
    mean_DO = mean
  ) %>%
...
temp_values <- gf2 %>%
  rename(
    temp_vel = velocity,
    temp_dvocc = dvocc,
    temp_trend = trend_per_year, # should change to trend_per_decade
    temp_grad = gradient,
    mean_temp = mean
  ) %>%
...
write.csv(vocc, file = paste0(
  "data/_newclim_", age, "/", age, "-w-do-nulls-", spp, "-", ssid_string, covs, "-", min(d$year), "-", max(d$year), ".csv"
))
