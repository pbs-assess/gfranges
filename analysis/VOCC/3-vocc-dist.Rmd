...
# max(gf2$sd, na.rm = T)
# [1] 1.12
# median(gf2$sd, na.rm = T)
# [1] 0.287
# 
# 2008 DATASET
mean(gf2$sd, na.rm = T)
# [1] 0.3274
temp_min_sd <- min(gf2$sd, na.rm = T)
# [1] 0.1035
max(gf2$sd, na.rm = T)
# [1] 1.248
temp_median_sd <- median(gf2$sd, na.rm = T)
# [1] 0.2882
...
lower_change_t <- c(Inf, do_sd_choice*2) 
upper_change_t <- c(temp_sd_choice*2, do_sd_choice*2) 
...
dvocc_d <- make_vector_data(list(d, d),
...
dvocc_b <- dvocc_b %>% mutate(dvocc = if_else(slope < 0, -km_per_decade, km_per_decade), trend=slope)
dvocc_b <- dvocc_b %>% mutate(dvocc = if_else(dvocc > (max_dist_all_years/ year_range)*10, 
  (max_dist_all_years/ year_range)*10, dvocc))
dvocc_b <- mutate(dvocc_b, dvocc = if_else(dvocc < -(max_dist_all_years / year_range)*10,
  -(max_dist_all_years / year_range)*10, dvocc))
...
gradbv <- plot_vocc(gf3t,
  ...
  title = "distance-based biotic velocity",
...
DO_values <- gf0 %>%
  ...
  select(x, y, DO_vel, DO_dvocc, DO_trend, DO_grad, mean_DO)
...
grad2t <- plot_vocc(vocc_t,
  ...
  title = "temp distance-based velocity",
  subtitle = paste0(min(d$year), "-", max(d$year))
)
