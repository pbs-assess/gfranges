...
if (!exists("nd_all")) {
  ...
  newcov <- readRDS("data/new_covariates.rds") %>% dplyr::select(X, Y, trawled, muddy, rocky)
...
if (update_spatial_model) {
  ...
  if (any(names(data) == "adult_density")) {
    ...
    adult_biomass <- sdmTMB::sdmTMB(data,
      adult_density ~ 0 + as.factor(year) + trawled + muddy + rocky,
    ...
    saveRDS(adult_biomass, file = paste0("data/", spp, "/model-mature-biomass-", spp, "-", ssid_string, "-priors-", priors, time1, "-newcovs.rds"))


    starttime2 <- Sys.time()
    imm_biomass <- sdmTMB::sdmTMB(data,
      imm_density ~ 0 + as.factor(year) + trawled + muddy + rocky,
      time_varying = ~ 0 + depth_scaled + depth_scaled2,
      time = "year", spde = spde,
      family = tweedie(link = "log"),
      ar1_fields = FALSE,
    ...
    saveRDS(imm_biomass, file = paste0("data/", spp, "/model-imm-biomass-", spp, "-", ssid_string, "-priors-", priors, time2, "-newcovs.rds"))
  } else {
    starttime3 <- Sys.time()
    total_biomass <- sdmTMB::sdmTMB(dat,
      density ~ 0 + as.factor(year)+ trawled + muddy + rocky,
      time_varying = ~ 0 + depth_scaled + depth_scaled2,
      time = "year", spde = spde,
      family = tweedie(link = "log"),
      ar1_fields = TRUE,
    ...
    saveRDS(total_biomass, file = paste0("data/", spp, "/model-total-biomass-", spp, "-", ssid_string, "-priors-", priors, "-newcovs.rds"))
  }
}
```
