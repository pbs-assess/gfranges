...
event_ratio <- event_maturity %>% group_by(fishing_event_id) %>% add_tally() %>%
  mutate(total_bio_mass = sum(new_mass, na.rm = TRUE), adult_mass_ratio = maturity_mass/total_bio_mass) %>% 
  filter(adult==1) %>% 
  select(fishing_event_id, total_bio_mass, adult_mass_ratio, adult_count, n) %>% unique()
