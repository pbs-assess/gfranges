...
ggsave(here::here("ms", "figs", "climate-maps-updated.pdf"), width = 6, height = 6)
...
p_temp_worm3 <- plot_chopstick_slopes(temp_vel_slopes, type = "temp", 
  legend_position = c(.25,.95)) + coord_flip(ylim =c(-14,7.95)) + 
  theme(axis.title.x = element_blank()) 
p_do_worm3 <- plot_chopstick_slopes(do_vel_slopes, type = "DO", 
  legend_position = c(.25,.95)) + coord_flip(ylim =c(-5.75,3.25)) +
  theme(axis.title.x = element_blank())
...
species_panels <- function (species, model, x_type, slopes, alpha_range = c(0.9,0.9)) {
  
 biotic_map <- filter(model$data, species == !!species) %>% plot_vocc(vec_aes = NULL,
    fill_col = "biotic_trend", fill_label = "", raster_cell_size = 4, na_colour = "lightgrey", white_zero = TRUE,
   ...
   ggtitle(paste0(species))  + theme(plot.margin = margin(0, 0, 0, 0.5, "cm"),
      axis.text = element_blank(), axis.ticks = element_blank(),
      axis.title.x = element_blank())
...
# species_panels ("mature Widow Rockfish", model, "temp", temp_slopes)
# species_panels ("mature Widow Rockfish", model, "DO", do_slopes)
# species_panels ("immature Widow Rockfish", model, "temp", temp_slopes)
# species_panels ("immature Widow Rockfish", model, "DO", do_slopes)

# species_panels ("immature Redbanded Rockfish", model, "temp", temp_slopes)
# species_panels ("mature Redbanded Rockfish", model, "DO", do_slopes)

A <- species_panels ("mature Canary Rockfish", model, "temp", temp_slopes, alpha_range = c(0.25,0.9))
B <- species_panels ("mature Redbanded Rockfish", model, "temp", temp_slopes)
C <- species_panels ("mature Shortspine Thornyhead", model, "temp", temp_slopes)

D <- species_panels ("mature English Sole", model, "temp", temp_slopes)
E <- species_panels ("mature Dover Sole", model, "temp", temp_slopes)
F <- species_panels ("mature Arrowtooth Flounder", model, "temp", temp_slopes)

G <- species_panels ("mature Shortspine Thornyhead", model, "DO", do_slopes)
H <- species_panels ("mature Sablefish", model, "DO", do_slopes)

I <- species_panels ("mature North Pacific Spiny Dogfish", model, "DO", do_slopes)
...
do_depth <- ggplot(do_data, aes(depth, do_est)) + 
  ...
  gfplot::theme_pbs() + theme(plot.margin = margin(0, 0.1, 0.2, 0, "cm"), 
...
model2 <- model2 %>% group_by(group) %>% 
  mutate(spp_count = length(unique(species))) %>% ungroup()
...
p_age_alone <- coef_scatterplot(filter(trendeffects, age == "mature"), coef = c("temperature","DO"), 
  ...
  # ggtitle("") +
  guides(colour=F) +
  geom_smooth(data = filter(trendeffects, coefficient != "DO" & age == "mature"), 
