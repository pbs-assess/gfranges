...
The inclusion of year as a factor centered each of the estimated spatiotemporal fields to have mean zero. Maximum likelihood estimation was conducted in R version 3.5.3 (TODO: R Core Team 2019) with the package sdmTMB (Anderson et al. In press, Anderson 2019) which interfaces automatic differentiation in Template Model Builder (TODO: Kristensen et al. 2016) with INLA (TODO: Rue et al. 2009).
...
Models exploring change as simple trends use Gaussian distributions, while velocities are heavy-tailed and therefore modeled using a student-t (degrees of freedom fixed at 7). Trend models converged with 500 knots and both temperature and DO, while velocity models would only converge with one environment variable at a time and 200 knots. <!-- TODO: make sure knots are explained somewhere... -->
In addition to checking the spatial distribution of model residuals, we simulated fake biomass trends for each species (using variance and spatial correlation parameters unique to each species) and reran each model 3 times <!-- TODO: simulated null models 10 times? --> and compared the distribution of effect sizes and number of sig effects in our simulated null models with the actual estimates.
