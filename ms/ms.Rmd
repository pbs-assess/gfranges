...
options(tinytex.clean = FALSE) # keep TeX auxiliary files for faster compiling
```
...
Our models suggest that from 2008 to 2018, seafloor temperature from late May to early August increased across almost the entirety of Canada's Pacific shelf region (95% interquantile range of `r signif(quantile(alldata$temp_trend, c(0.025, 0.975))[[1]], digits = 2)` to `r signif(quantile(alldata$temp_trend, c(0.025, 0.975))[[2]], digits = 2)`), with warming tending to be most pronounced in shallower areas (Fig. \@ref(fig:climate-maps)C).
<!-- TODO: should letters be added and included here? eg. "C" -->
<!-- SA: yeah, might be helpful for referring to specific panels here-->
In contrast, change in seafloor DO was much more variable in the direction of change (95% interquantile range of `r signif(quantile(alldata$DO_trend, c(0.025, 0.975))[[1]], digits = 2)` to `r signif(quantile(alldata$DO_trend, c(0.025, 0.975))[[2]], digits = 2)`), with the greatest increases also occurring in the shallowest areas and the greatest decreases occuring across a wide range of depths (Fig. \@ref(fig:climate-maps)D).
When these local changes are placed in their geographic context in the form of a gradient-based velocity estimate, the majority of the highest velocities occur in the bottom of troughs in Queen Charlotte Sound and Dixon Entrance (Fig. \@ref(fig:climate-maps)E, F).

Geostatistical models exploring either trend- or velocity-based indices of change resolved similar overall interactions for both temperature and DO, but the overall main effects of temperature change and mean DO differed (Fig \@ref(fig:global-coefs) TODO and \@ref(fig:worm-plots)) if both model types included).
...
Despite this difference, the qualitative patterns for most species and maturity class tended to agree between model types (Figs. \@ref(fig:trend-chopsticks) and \@ref(fig:vel-chopsticks)); therefore, we focus on interpreting the results of the trend-based model (Fig. \@ref(fig:worm-plots) <!-- TODO: add A, if both models are in this plot-->).
Null models based on simulated biotic trends showed slightly more significant relationships than would be expected by chance, but far fewer than the model based on modeled changes in abundance
<!-- SA: what is this "modeled changes in abundance"? you mean density? even still I'm not sure -->
and all the effect sizes for parameters of interest were much nearly an order of magnitude smaller (Fig. TODO: add supplementary fig).
<!-- \@ref(fig:null-coefs) -->
Therefore, we explore both the overall and species-level patterns, but keep in mind that a slightly higher percentage than the stated alpha level of 0.05 (TODO %) may be spurious.
...
<!-- SA: I think it's fine to leave the gnarly complicated thing in the supplement; maybe we can just add the global effect to the overall coefficient plot at the bottom? maybe emphasized with something fancy like a diamond?-->

<!-- SA: I wonder if each of these main findings should be split off into its own paragraph with the point as a topic sentence? This is a super dense results paragraph -->
Across species, the most common patterns were that
(1) the impact of warming on biomass change was more negative (or less positive) in locations with higher mean temperatures than in cooler locations (e.g., Widow Rockfish, mature Shortspine Thornyhead, and mature English and Flathead Sole),
(2) the impact of increasing DO levels was more negative in locations with lower mean DO levels than in locations with higher mean DO levels (e.g., Dogfish, mature Sablefish, Widow Rockfish, and English and Flathead Sole), 
<!-- Note that negative effect of DO change is sig overall, but not interaction... so may change this? -->
and (3) overall immature abundances showed a tendency to respond more positively to both mean DO and changes in DO (Fig. \@ref(fig:supp-global-coefs)).
There were no strong patterns of similarity amoungst species belonging to the same genus or family
<!-- (TODO: should we mention here that they were included as random effects in trial models, but not the final model?)  -->
<!-- SA: yeah, we should probably mention it in the methods -->
as illustrated by relationships with biotic trends that were negative at high temperatures (Canary Rockfish and English Sole) or positive at low temperatures (e.g., Widow Rockfish and Flathead Sole) occuring in members of both the largest families represented in our analysis, Sebastidae (rockfish) and Pleuronectidae (righteye flounders; Fig. \@ref(fig:worm-plots) and \@ref(fig:species-map-chop-panel)).
<!-- SA: it occurs to me that we need to get the latin names and for all the species and that will make the text get kind of ridiculous. I wonder if we should have a table with all the species in latin names and just refer to that? -->
...
Overall climate sensitivities correlated with life-history traits and were strongest for the youngest populations (Fig. \@ref(fig:coef-scatterplots)).
...
Consistent with this finding, the strongest negative effects of warming (estimated for the warmest parts of a species distribution) were for species occurring at intermediate depths, whereas most species with mean encounter depths deeper than 290 m appeared to increase in density with greater warming (Fig. \@ref(fig:slope-by-depth) TODO: add panel letter A).
In contrast, both strong negative and positive responses to changes in DO occurred across a range of depth preferences, although increasing DO in the lowest DO locations is mostly negative for species averaging shallower than 150 m (Fig. \@ref(fig:slope-by-depth) TODO: add panel letter B).
...
<!-- SA: Can we capitalize the first letter of the panel labels here and throughout? -->

```{r climate-maps, fig.cap="Maps of predicted mean conditions (top row), decadal trends (middle row), and decadal velocities (bottom row) of bottom temperature and dissolved oxygen for 2008--2018. Values are estimated using geostatistical spatiotemporal models of CTD sensor data collected during late spring/early summer groundfish trawl surveys. Bathymetry lines at every 100 m are overlaid in shades of grey that increase with depth.", out.width="5in"}
include_graphics(here("ms", "figs", "climate-maps-updated.png"))
```

\clearpage

<!-- SA: "only included when the confidence intervals on the slopes did not overlap" <- can we always include the CIs? -->

<!-- PE: The line you quote was meant to convey something different, so I've edited it to make its intent clearer. The CI have always been included, so those that don't show up are too small to extend outside the dots...
-->

<!-- SA: Am I reading it right that Bocaccio really don't like it warming? That's interesting considering they've had one of the biggest recruitment events ever seen for rockfish in this warming decade. -->

<!-- SA: Point of discussion: if I'm reading it right, Pacific Cod don't like it warming. Tie into collapse in Alaska with warm blob -->

```{r worm-plots, fig.cap="Slopes of relationships between climate trends and percent change in biomasss controling for effect of the other climate variable for groundfish species. Open circles indicate patterns for immatures, while closed circles represent individuals large enough to have a 5\\% chance of being reproductively mature or belonging to species for which maturity data was not available. Black indicates the overall effect when the 95\\% CI of the estimated interaction with mean climate condition overlapped zero. Colours in legends indicate slopes estimated for the low (0.025) and high (0.975) quantiles of mean climate conditions. Coefficients are ordered by mean slope for each species within each panel.", out.width="5.2in"}
include_graphics(here("ms", "figs", "worm-plot-trend.pdf"))
# TODO: decide if velocity slopes belong here, removed them for now (04-23)
# TODO: should species be in same order on both trend and velocity panels?
# include_graphics(here("ms", "figs", "worm-plot-both.pdf"))
```

<!-- SA: probably need to reduce the number of examples for the main figures in the next one -->
<!-- SA: writing out the caption like this lets us reference other figures or citations -->

(ref:chop-panel) Maps of biotic (top), temperature (middle), and DO (bottom) trends underlying interaction plots (between map rows) of predicted biotic trend (y-xis) for each standard deviation change in seafloor temperature and DO (x-axis) for mature populations of species representing a range of taxonomic groups and response patterns. Colours and lines in the interaction plots match the slopes represented in Fig. \@ref(fig:worm-plots), where low (0.025) and high (0.975) quantiles of mean climate conditions are illustrated in blue and red for temperatures and yellow and purple for DO.
Black lines are shown when TODO.
As an example, for Widow Rockfish, we estimate that biomass density has increased with increasing temperatures in cooler regions and decreased with increasing temperatures in warmer regions.

<!-- SA: I almost wonder if we should show these chopstick plots before the moderately overwhelming slopes for all the species. Not sure, I can see an argument either way. Maybe at least allude in the caption for the coefficient plot that this next plot exists and may help with interpretation.

The alternative would be to help the reader along by giving some examples like in this chopstick plot, and then zooming out to show the coefficients for all the species.
-->
<!-- PE: I like the idea of putting the chops first... I'll see how that works... -->


\begin{landscape}

```{r species-map-chop-panel, fig.cap="(ref:chop-panel)", out.width="8.2in"}
include_graphics(here("ms", "figs", "species-map-chop-panel.png"))
```

\end{landscape}

```{r slope-by-depth, fig.cap="Slopes of the relationship between changes in biomass and climate for the environmental extremes expected to have the greatest physiological impact---(A) highest temperatures and (B) lowest DO---plotted against each species' mean depth of occurrence. (C) Smoothed (solid lines represent a GAM) relationship between estimated temperature (right axis; blue) and DO (left axis; purple) with mean depth in each 4x4 km survey grid cell (dots). An increase in variablity of slopes for changes in DO occurs for species whose mean depth is at or below depths where estimated mean DO levels approach hypoxic conditions (dashed line at <1.4 ml/L in panel C).", out.width="4in"}
include_graphics(here("ms", "figs", "slope-by-depth.png"))
```

<!--
SA: can we spell out what a negative (or positive) slope means here again to remind people?
Negative slope means that...
easy to lose the thread

and add panel letters obviously

am I reading it correctly that negative slopes mean that as DO goes up, biomass goes down? Can we spell that out again?

if so, does that mean that the slopes in the bottom right of panel B are particularly perplexing? near to hypoxic conditions and seemed to like it more hypoxic?

I guess that points to hypoxia not being the main driver and just that these really deep species may be more O2 driven as a proxy for other things?
-->

<!-- \begin{landscape} -->

```{r coef-scatterplots, fig.cap="Main effect coefficients of the relationship between biotic trends and changes in climate for species with different life history strategies. Mean age and depth are calculated across all sampled individuals of each maturity class (as defined by the length at 50\\% maturity) within a species. Growth rate is estimated as the length at maturity for females divided by the 0.95 quantile of all otolith or fin-ray ages from immature females of each species. Separate regression slopes are plotted for each maturity class except in relation to mean age given that maturity is largely a function of age.", out.width="\\textwidth"}
include_graphics(here("ms", "figs", "coef-scatterplots.pdf"))
```

<!-- \end{landscape} -->

<!-- SA: I went into the code and capitalized the xlab and ylabs for this plot. Can you also do the facet labels? 
Same for other figures (at least the main paper ones; supplement doesn't matter as much)
--> 

<!-- PE: Titles in main figures have capitals now.  Should legend groups like "high temp" and "immature" be capitals too? I think not, but... -->

<!-- SA: I'm not sure I follow the intent of the immature growth rate calculation -->
...
(ref:fig-vel-chopsticks) Predicted interactions from separate models predicting biotic \textbf{velocity} based on seafloor temperature \textbf{velocity} and the mean temperature, or seaflour DO \textbf{velocity} and mean DO levels, while only controlling for mean local log(biomass) in both cases. Colours and symbols are the same as in Fig. \@ref(fig:trend-chopsticks).

\begin{landscape}

```{r vel-chopsticks, fig.cap="(ref:fig-vel-chopsticks)", out.width="7.5in"}
