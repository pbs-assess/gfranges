...
This difference is likely due to covariation between temperature and DO within our dataset <!-- TODO: run temp only trend model to check if this is true -->, which the trend model could control for, but the the velocity one could not. Despite this difference, the qualitative patterns for most species and maturity class tended to agree between model types (TODO: cite supplementary materials <!--\@ref(fig:trend-chopsticks), \@ref(fig:vel-chopsticks)-->); therefore, we will focus on interpreting the results of trend-based model (Fig. \@ref(fig:worm-plots) <!-- TODO: add A, if both models are in this plot-->). 
Across species, the most common pattern was that 1) the impact of warming on biomass change was more negative (or at least less positive) in locations with higher mean temperatures than in cooler locations (eg. Widow Rockfish, mature Shortspine Thornyhead, and mature English and Flathead Sole), 2) the impact of increasing DO levels was more negative in locations with lower mean DO levels than in locations with higher mean DO levels (eg. Dogfish, mature Sablefish, Widow Rockfish, and English and Flathead Sole), and 3) overall immature abundances were more likely than mature abundances to show negative relationships (eg. Sablefish, Widow Rockfish, and Dover Sole (\@ref(fig:trend-chopsticks)). 
<!-- TODO: checking this with model including age as fixed effect (mature biomass as the intercept)...--> 
There appeared to be no strong patterns of similarity amoungst species belong to the same genus or family (tested as potential random effects, but not included in the final model) as illustrated by negative at high temperatures (Canary Rockfish and English Sole) or positive at low temperatures (eg. Widow Rockfish and Flathead Sole) impacts on biotic trends being exibited by members of both the largest families represented in our analysis Sebastidae (rockfish) and Pleuronectidae (righteye flounders; Fig. \@ref(fig:species-map-chop-panel)).   
