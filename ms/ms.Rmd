...
In addition to checking the spatial distribution of model residuals, we simulated fake biomass trends for each species (using variance and spatial correlation parameters unique to each species) and reran each model 3 times <!-- TODO: simulate null model 10 times? --> and compared the distribution of effect sizes and number of sig effects in our simulated null models with the actual estimates. These simulated biomass trends were divided by the real spatial gradients in biomass in order to simulate biotic velocities. 
...
Separate geostatistical models exploring either trend- or velocity-based indices of change resolved similar overall interactions for both temperature and DO (TODO: cite supplementary materials <!--\@ref(fig:global-coefs)--> and (Fig. \@ref(fig:worm-plots)) if both model types included), but the overall main effects of temperature change and mean DO did differ. 
This difference is likely due to covariation between temperature and DO within our dataset <!-- TODO: run temp only trend model to check if this is true -->, which the trend model could control for, but the the velocity one could not. Despite this difference, the qualitative patterns for most species and maturity class tended to agree between model types (TODO: cite supplementary materials <!--\@ref(fig:trend-chopsticks), \@ref(fig:vel-chopsticks)-->); 
therefore, we will focus on interpreting the results of trend-based model (Fig. \@ref(fig:worm-plots) <!-- TODO: add A, if both models are in this plot-->). 
Across species, the most common patterns were for 1) the impact of warming on biomass change was more likely to be negative (or at least less positive) in locations with higher mean temperatures than in cooler locations, and while controling for the effect of temperature 2) the impact of increasing DO levels  was negative in locations with lower mean DO levels than in locations with higher mean DO levels. 
...
```{r global-coefs, fig.cap="Overall coefficient estimates from each model.", out.width="4in"}
