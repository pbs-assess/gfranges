...
We model both biomass and climate as either velocities or as raw trends (Fig. \@ref(fig:climate-maps)). In addition we attempt to assess the importance of the gradient component in velocity estimates by adding gradient to the model based on trends.
...
<!-- TODO: remaining figures to consider
...
```{r climate-maps, fig.cap="Maps of predicted trends (top panels) and velocities (bottom panels) of change for bottom temperature and dissolved oxygen during the trawl surveys conducted between 2008 to 2018. Values are estimated from a spatiotemporal models of CTD sensor data collected during late spring/early summer groundfish trawl surveys.", out.width="5in"}
include_graphics(here("ms", "figs", "climate-maps-updated.pdf"))
```

\clearpage

```{r worm-plots, fig.cap="Slopes of relationships between climate trends and percent change in biomasss for groundfish species. Open circles indicate patterns for immatures, while closed circles represent individuals large enough to have a 5 percent chance of being reproductively mature or belonging to species for which maturity data was not available. Black indicates overall slope where the estimated interaction with mean climate condition overlapped zero. Colours indicate slopes estimated for the low (0.025) and high (0.975) quantiles of mean climate conditions (only included when the confidence intervals on the slopes did not overlap).", out.width="6in"}
...
Arrowtooth or Dover - warming good when it's cool, neutral when warm 
sablefish - warming good, increasing DO not

-->


```{r temp-chops, fig.cap="Temperature ...", out.width="6in"}
# include_graphics(here("ms", "figs", "temp-chops.pdf"))
```

\clearpage

```{r do-chops, fig.cap="Dissolved oxygen ...", out.width="6in"}
# include_graphics(here("ms", "figs", "do-chops.pdf"))
```

\clearpage

```{r slope-by-depth, fig.cap="Variation in slopes across species with different mean depths of occurrence.", out.width="4in"}
include_graphics(here("ms", "figs", "slope-by-depth.pdf"))
```

\clearpage

<!-- TODO: May be we should change this to mean age of mature fish sampled for each species? That would make it easier to relate values to the timespan of the study. 
-->

```{r coef-by-max-age.pdf, fig.cap="The maximum lifespan of the species may influence the relationships between climate trends and biomass change.", out.width="4in"}
include_graphics(here("ms", "figs", "coef-by-max-age.pdf"))
```
...
-   all the chopstick panels both trend and velocity

<!-- TODO: change orientation to landscape? -->

\begin{landscape}

```{r trend-chopsticks, fig.cap="Caption goes here", out.width="11in"}
include_graphics(here("ms", "figs", "supp-trend-chopsticks.pdf"))
```

\clearpage

```{r vel-chopsticks, fig.cap="Caption goes here", out.width="11in"}
...
-   other coefs plot
<!-- TODO: This fig might go here instead of in results -->

```{r supp-coef-scatterplots, fig.cap="Caption goes here", out.width="6in"}
include_graphics(here("ms", "figs", "supp-coef-scatterplots.pdf"))
```

-   pairs plot of predictors

-   plot vocc regression residuals against the main predictors... (and not in model and fitted vs. residuals plot; but don't necessarily include in paper)
